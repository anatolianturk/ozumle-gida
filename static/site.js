...
const translations = {
  tr: {
    ...
    links: {
      ...
      contact: "İletişim",
      distanceSalesAlt: "Uzaktan Satış Sözleşmesi",
      privacyAlt: "Gizlilik Politikası",
      sitemap: "Site Haritası",
    },
    btns: {
      addToBasket: "Sepete Ekle",
      hideBasket: "Sepeti Gizle",
      showBasket: "Sepeti Göster",
      emptyBasket: "Sepeti Boşalt",
      quantity: "Adet",
      orderViaWhatsapp: "Whatsapp'dan Siparişini İlet",
    },
    cart: {
      productTotal: "Ürün Tutarı",
      vatIncluded: "(KDV Dahil)",
      freeShippingNote: "15 kg ve üzeri siparişlerde kargo bedavadır.",
      shippingCost: "Kargo Ücreti:",
      shippingTaxNote: " TL (Vergiler Dahil)",
  ...
  en: {
    ...
    links: {
      ...
      products: "Our Products",
      about: "About Us",
      story: "Our Flavor Story",
      distanceSales: "Distance Sales Agreement",
      privacy: "Privacy Policy",
      kvkk: "KVKK Information Notice",
      contact: "Contact",
      distanceSalesAlt: "Remote Sales Agreement",
      privacyAlt: "Privacy Policy",
      sitemap: "Site Map",
    },
    btns: {
      ...
      quantity: "Quantity",
      orderViaWhatsapp: "Send Your Order via WhatsApp",
    },
    cart: {
      productTotal: "Product Total",
      vatIncluded: "(VAT Included)",
      freeShippingNote: "Free shipping for orders 15 kg and above.",
...
const currentLang = getLangFromSubdomain();
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (IS_HOME) {
    let part = imgWithBtn(
      "/static/img/pages/header.jpg",
      SITE.headSloganBtn,
      SITE.headSloganLnk,
      [SITE.headImgSloganStart, SITE.headImgSloganEnd]
    );
    if (IS_MOBILE) {
      part.style.marginTop = "-128px";
    } else {
      part.style.marginTop = "-48px";
    }
    $body.append(part);
  } else {
  ...
  if (window.location.href.includes("/products/")) {
    ...
    if (product) {
      ...
      console.error("Product not found: " + pun);
    }
  }
  ...
  setTimeout(function () {
    ...
    for (let [key, value] of prms.entries()) {
      ...
      if (product) {
        addToBasket(key, parseInt(value));
      }
    }
  }, 987);
});
...
function getData(p) {
  // Extract subdomain (default to 'tr' if not present)
  const hostParts = window.location.hostname.split(".");
  const subdomain = hostParts.length > 2 ? hostParts[0] : "tr";

  // Fallback to 'tr' if subdomain is unknown
  const lang = ["en", "tr"].includes(subdomain) ? subdomain : "en";

  const xhr = new XMLHttpRequest();
  xhr.open("GET", `/data/${lang}/${p}.json`, false);
  xhr.send();

  if (xhr.status === 200) {
    return JSON.parse(xhr.responseText);
  } else {
    console.error(`Error fetching ${lang}/${p}.json: `, xhr.statusText);
    return null;
  }
}

function lnk(href, key, blank = false) {
  const translatedText = lang?.links?.[key] || key;

  const a = document.createElement("a");
  a.href = href;
  a.textContent = translatedText;
...
function imgWithBtn(s, t, u, l) {
  ...
  b.addEventListener("click", function () {
    window.location.href = u + window.location.search;
  });
...
function basketAdder(prevElem, prdId) {
  ...
  const btnText = lang.btns.quantity;

  let $q = em(exi.quantity + "  " + btnText);
...
function refreshBasket() {
  ...
  if (BASKET.length > 0) {
    ...
    const totalText = lang.cart.productTotal;
    let $pTotal = p("");
    $pTotal.id = "pTotal";
    const vatIncluded = lang.cart.vatIncluded;
    $pTotal.textContent =
      totalText + " : " + formatPrice(total) + " " + vatIncluded;
    frag.append($pTotal);

    const freeShippingNote = lang.cart.freeShippingNote;
    let $e = em(freeShippingNote + ".");
    ...
    let ship = calcShip(w);
    if (w < 15) {
      const shippingCost = lang.cart.shippingCost;
      const shippingTaxNote = lang.cart.shippingTaxNote;
      let $c = p(shippingCost + ": " + ship + " " + shippingTaxNote);
      frag.append($c);
      frag.append($e);
    } else {
      const shippingFree = lang.cart.shippingFree;
      $e.textContent = shippingFree;
      frag.append($e);
    }
    ...
    $total.id = "total";
    const grandTotal = lang.cart.grandTotal;
    $total.textContent = grandTotal + " : " + formatPrice(total + ship);
    frag.append($total);

    $bi.querySelector("em").textContent = formatPrice(total);
    const btnText = lang.btns.orderViaWhatsapp;
...
function doBasket($body, $f) {
  ...
  const showBasket = lang.btns.showBasket;

  let $bs = btn(showBasket);
  $bs.id = "btnShowBasket";
  ...
  const emptyBasketBtn = lang.btns.emptyBasket;
  let $be = btn(emptyBasketBtn);
...
function doProductInner($p, prd, isLinked) {
  ...
  $pr.innerHTML = `${prd.price} TL <em> (${vatIncluded})</em>`;
  $p.append($pr);
...
function doProduct(product, isLinked = true) {
  const btnText = lang.btns.addToBasket;
