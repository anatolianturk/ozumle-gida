...
var IS_MOBILE = (function () {
  ...
  var mobileRegex =
    /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini|Mobile|Mobi|iOS|CriOS|FxiOS|CFNetwork|UCBrowser|Silk|Kindle|Tablet|Mac.*Mobile|MacIntel|Phone|samsung|SAMSUNG/i;
...
function getData(p) {
  ...
  const subdomain = hostParts.length > 2 ? hostParts[0] : "en";
...
function lnk(href, key, blank = false) {
  const translatedText = localeData?.links?.[key] || key;
...
function basketAdder(prevElem, prdId) {
  ...
  const btnText = localeData.btns.quantity;
...
function refreshBasket() {
  ...
  if (BASKET.length > 0) {
    ...
    const totalText = localeData.cart.productTotal;
    let $pTotal = p("");
    $pTotal.id = "pTotal";
    const vatIncluded = localeData.cart.vatIncluded;
    $pTotal.textContent =
      totalText + " : " + formatPrice(total) + " " + vatIncluded;
    frag.append($pTotal);

    const freeShippingNote = localeData.cart.freeShippingNote;
    ...
    let ship = calcShip(w);
    if (w < 15) {
      const shippingCost = localeData.cart.shippingCost;
      const shippingTaxNote = localeData.cart.shippingTaxNote;
      let $c = p(shippingCost + ": " + ship + " " + shippingTaxNote);
      frag.append($c);
      frag.append($e);
    } else {
      const shippingFree = localeData.cart.shippingFree;
...
function doBasket($body, $f) {
  ...
  const showBasket = localeData.btns.showBasket;
  ...
  const emptyBasketBtn = localeData.btns.emptyBasket;
...
function showBasket() {
  ...
  const hideBasketText = localeData.btns.hideBasket;
...
function hideBasket() {
  ...
  const showBasketText = localeData.btns.showBasket;
...
function doProduct(product, isLinked = true) {
  const btnText = localeData.btns.addToBasket;
